...
def _display_clock_frame(frame: pd.DataFrame) -> pd.DataFrame:
    """Remove timezone metadata only for display so all Lunch hours stay aligned."""
    if not isinstance(frame, pd.DataFrame) or frame.empty:
        return frame
    work = frame.copy(deep=False)
    changed = False
...
def _display_table(
    ...
    *,
    ...
    if not isinstance(frame, pd.DataFrame) or frame.empty:
        ...
        return
    st.dataframe(_display_clock_frame(frame), use_container_width=True, hide_index=True, height=height)
    if historical:
...
def render_lunch_six_core_fields(*, state: MutableMapping[str, Any] | None = None) -> None:
    ...
    try:
        ...
        time_cols[1].metric("Current UTC / Broker", str(fresh.get("broker_clock_display") or fresh.get("current_utc_display") or "-"))
        lag = fresh.get("lag_minutes")
        time_cols[2].metric("Latest Loaded Candle", str(fresh.get("latest_loaded_display") or "No loaded candle").replace(" UTC", ""), f"{lag:g} min lag" if isinstance(lag, (int, float)) else "No timestamp")
    except Exception:
        pass
