    ...
    *,
    ...
    if broker_clock:
        try:
            ...
            return frame_to_shared_broker_clock(work, state or {}, canonical=_canonical(dict(state or {})))
        except Exception:
            pass
...
def _latest_completed_h1_from_state(state: MutableMapping[str, Any], result: Mapping[str, Any] | None = None) -> Any | None:
    ...
    try:
        ...
        return shared_broker_time_provider(state, canonical=_canonical(state)).get("latest_broker_candle_utc")
    except Exception:
...
def _render_history_sync_status(state: MutableMapping[str, Any], history_frame: pd.DataFrame | None = None) -> None:
    """Show the exact broker/history watermark comparison requested by V5."""
    try:
        ...
        st.markdown("#### History Sync Status")
        cols = st.columns(4)
        if report.get("synced"):
            cols[0].success("GREEN — Synced")
        else:
            cols[0].error("RED — Out of Sync")
        cols[1].metric("Latest Broker Candle", str(report.get("shared_broker_time_display") or "Not available"))
        cols[2].metric("Latest History Record", str(report.get("latest_history_record_display") or "Not available"))
        diff = report.get("difference_minutes")
        cols[3].metric("Difference Minutes", f"{float(diff):.2f}" if isinstance(diff, (int, float)) else "-")
