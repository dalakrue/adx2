...
def render_canonical_copy_export(
    *,
    ...
    try:
        if prepare_short:
            text, stats = build_short_payload(canonical, summary, plan, state)
        ...
        if prepare_all:
            ...
            text = all_text(canonical, summary, plan, readiness, state)
...
def render_direct_canonical_copy_buttons(
    *,
    ...
    try:
        short_payload, short_stats = build_short_payload(canonical, summary, plan, state)
        from ui.copy_tools import central_copy_button
        if include_full:
            full_payload = all_text(canonical, summary, plan, readiness, state)
