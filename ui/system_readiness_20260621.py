...
def render_system_readiness(*, state: MutableMapping[str, Any] | None = None) -> Mapping[str, Any]:
    ...
    # Preserve the existing copy/export control behind an explicit display-only
    # toggle without cluttering the new research-history purpose of Field 6.
    if st.toggle("Show existing canonical copy/export controls", value=False, key="field6_existing_copy_export_20260621"):
        try:
            from ui.canonical_copy_export_20260619 import render_canonical_copy_export
            render_canonical_copy_export(state=state, readiness=snapshot, location="field6")
        except Exception as exc:
            st.caption(f"Copy/export controls skipped safely: {exc}")
    return snapshot
