...
SUBPAGE_PARENT: Dict[str, str] = {
    **{x: "Lunch" for x in LUNCH_CHILDREN},
    "Regime + Combined Logic": "Lunch", "Unified Regime + Logic": "Lunch", "Regime Summary": "Lunch", "Combine Logic": "Lunch", "Combined Logic": "Lunch",
...
def _normalize_page(page: str | None) -> str:
    ...
    aliases = {"Home": "Lunch", "Data Visualization": "Lunch", "AI Assistant": "Research", "Metric": "Lunch", "Doo Prime": "Morning", "Regime": "Lunch", "Dinner": "Lunch"}
...
def _sync_legacy_state(page: str, subpage: str = "") -> Tuple[str, str]:
    ...
    if raw_page == "Dinner":
        st.session_state["lunch_focus_field_20260619"] = 6 if subpage == "AI Assistant" else 5
        page, subpage = "Lunch", ""
    page = _normalize_page(page)
    if subpage in SUBPAGE_PARENT:
        page = SUBPAGE_PARENT[subpage]
        if subpage in DINNER_CHILDREN or subpage in {"Unified Regime + Logic", "Regime Summary", "Combine Logic", "Combined Logic"}:
            st.session_state["lunch_focus_field_20260619"] = 6 if subpage == "AI Assistant" else 5
            subpage = ""
...
def safe_antd_navigation(location_key: str = "antd_main_navigation") -> Tuple[str, str]:
    ...
    if selected == "AI Assistant":
        st.session_state["lunch_focus_field_20260619"] = 6
        return _sync_legacy_state("Lunch", "")
