...
def _render_market_time_metrics(*, query_mt5: bool = True) -> dict[str, Any]:
    ...
    cols[4].metric("Latest Loaded Candle", str(snap.get("latest_loaded_display") or "No loaded candle").replace(" UTC", ""), delta)
    st.caption("MetaTrader tick timestamps are UTC. The broker-clock display uses the UTC offset configured below; latest loaded candle is the actual dataframe timestamp used for freshness checks.")
    return snap
...
def _render_settings() -> None:
    ...
    with st.expander("Open / Close — Twelve Data + MT5 Market Connector", expanded=True):
        ...
        st.number_input(
            ...
            help="MT5 Python tick timestamps are UTC. Set the offset shown on your broker chart so the broker-clock metric matches MetaTrader without altering candle data.",
        )
        try:
