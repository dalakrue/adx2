...
def render_lunch_quick_decision() -> None:
    """Render Quick Decision, cached search, then exactly six principal fields."""
    if not _lunch_route_active():
        return
    started = time.perf_counter()
    ...
    if summary:
        render_eight_cards(summary, location="lunch_quick_decision_20260621")
    ...
    if st.session_state.pop("lunch_calculation_completed_notice_20260621", False):
        st.success("Calculation completed once. The canonical result is saved and Lunch Field 1 is open.")

    from ui.lunch_search_20260621 import render_lunch_search
    render_lunch_search(state=st.session_state)
    st.markdown("---")
