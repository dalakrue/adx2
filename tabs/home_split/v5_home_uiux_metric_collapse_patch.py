...
def install(g: dict) -> None:
    ...
    def _current_panel_collapsed(results):
        with st.expander("🧭 Open Current Data Session Intelligence", expanded=False):
            if callable(base_current_panel):
                base_current_panel(results)

    def _one_hour_collapsed(results):
        with st.expander("🔔 Open One-hour exit opportunity sound rule", expanded=False):
            if callable(base_one_hour):
                base_one_hour(results)
