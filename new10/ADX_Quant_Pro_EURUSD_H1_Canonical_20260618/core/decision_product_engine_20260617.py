...
def validate_data_quality(
    ...
    if future_rows:
        blocking.append(f"Future timestamps detected and rejected: {future_rows}")
...
def build_decision_result(
    *, legacy_shared: Dict[str,Any], ohlc: pd.DataFrame, symbol: str="EURUSD", timeframe: str="H1",
    ...
    if label=="LARGE DISAGREEMENT":
        final_blockers.append("large multi-horizon disagreement")
    ...
    if forecast_conflict:
        final_blockers.append(f"PowerBI/prediction {primary.direction} conflicts with Full Metric H1 {market_view}")
    elif primary.decision == "WAIT":
        final_blockers.extend(primary.blocking_reasons)
    elif primary.decision in {"BUY", "SELL"} and market_view in {"BUY", "SELL"} and primary.decision != market_view:
        final_blockers.append(f"Forecast tradeability {primary.decision} cannot reverse Full Metric H1 {market_view}")
    ...
    if market_view in {"BUY", "SELL"} and m1_status == "WAIT FOR CONFIRMATION":
        final_blockers.append("M1 timing conflicts with H1; wait for confirmation")

    can_trade_authority = market_view in {"BUY", "SELL"} and authority_tradeability == market_view
    can_trade_forecast = primary.decision == market_view and not forecast_conflict and label != "LARGE DISAGREEMENT"
    can_trade_m1 = m1_status != "WAIT FOR CONFIRMATION"
    if quality.status == "FAIL_ALL":
        final_decision = "DATA NOT READY"
    elif can_trade_authority and can_trade_forecast and can_trade_m1:
    ...
    supporting=[f"Full Metric H1 market view {market_view}",f"{selected}h forecast confirmation {primary.direction}",f"horizon agreement {agreement_score:.0%}",f"priority {priority.label} {priority.score:.1f}"]
