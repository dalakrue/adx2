...
def install(g):
    ...
    def _render_doo_finder(results):
        ...
        if view_mode == "Selected hour":
            target_start = pd.Timestamp(chosen_date) + pd.Timedelta(hours=int(str(hour_label).split(":")[0]))
