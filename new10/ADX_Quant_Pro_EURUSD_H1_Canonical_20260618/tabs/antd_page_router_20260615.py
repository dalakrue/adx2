...
def _render_dinner(ns: dict, subpage: str) -> None:
    ...
    prev = _prev_data(ns)
    options = ["Regime Summary", "Combined Logic", "AI Assistant"]
    requested = "AI Assistant" if str(subpage) == "AI Assistant" else "Regime Summary" if str(subpage) == "Regime Summary" else "Combined Logic"
    widget_key = "dinner_inner_tab_20260618"
    if st.session_state.get(widget_key) not in options or str(subpage) in {"AI Assistant", "Unified Regime + Logic", "Regime Summary", "Combine Logic", "Combined Logic"}:
    ...
    from tabs.dinner_unified_center_20260617 import render_dinner_combined_logic, render_dinner_regime_summary
    from ui.decision_product_panel_20260617 import render_regime_lifecycle_panel
    renderer = render_dinner_regime_summary if selected == "Regime Summary" else render_dinner_combined_logic
    _safe_component(
        selected, renderer, ns, prev, render_regime_lifecycle_panel,
    )
