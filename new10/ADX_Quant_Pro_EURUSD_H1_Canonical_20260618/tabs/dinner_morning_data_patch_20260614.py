...
def _render_chatgpt_style_ai() -> None:
    ...
    if send:
        ...
        try:
            ...
            answer = format_grounded_answer(canonical_grounding, answer)
        except Exception:
            pass
...
def _render_dinner(ns: dict, prev_data) -> None:
    ...
    active = st.session_state.get("dinner_active_inner_20260615", "Combined Logic")
    if active not in options:
        active = "Combined Logic"
    try:
    ...
    if active == "AI Assistant":
        ...
        if active == "Regime Summary":
            render_dinner_regime_summary(ns, prev_data)
        else:
            render_dinner_combined_logic(ns, prev_data)
