...
def _render_run_gate(ns: dict, location: str) -> None:
    active_page = str(st.session_state.get("active_page") or st.session_state.get("tab_choice") or "")
    if active_page in {"Dinner", "Regime"}:
        ...
        if _run_ready():
            st.caption(f"Main Settings run is active and synchronized • Run ID {run_id or 'legacy cached run'}. No inner Run/Stop control is required.")
        else:
            st.info("Run Calculation in Settings once; this inner section will load automatically from that same run.")
        return
...
def _render_regime_intelligence_center(ns: dict) -> None:
    ...
    if not _run_ready() and not isinstance(st.session_state.get("regime_context_20260614"), dict):
        st.info("Run Calculation once in Settings. Regime Summary and every inner section will then open from the same synchronized result without another Run or Load button.")
        return
...
def _final_summary_copy(ns: dict, compact: bool = False) -> str:
    ...
    regime = _regime_sync_snapshot(rctx) if rctx else {"Status": "Run Calculation in Settings first"}
