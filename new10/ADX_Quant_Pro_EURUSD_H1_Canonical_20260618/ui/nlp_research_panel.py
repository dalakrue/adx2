...
def _fetch_if_connected(force: bool) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if isinstance(fetched, list):
            finnhub_rows = [row for row in fetched if isinstance(row, dict)]
    except Exception:
        pass
    merged = _merge_real_articles(finnhub_rows, cached)
    # Only explicit Analyze/Refresh button actions call this function. Fill a
    # sparse one-article cache with public RSS evidence so the ranked table can
    # reach its minimum target using real articles, never generated placeholders.
    if len(merged) < MIN_RANKED_NEWS_ROWS:
        merged = _merge_real_articles(merged, _fetch_public_rss_news(limit=100))
    try:
...
def render_nlp_research_workspace(selected: str = "NLP") -> None:
    ...
    if selected == "NLP":
        ...
        # The only API-key inputs remain in Settings and the optional sidebar.
        ...
        _config_controls()
        if not current:
            st.info("Finnhub is optional. Connect it in Settings/sidebar, or analyze existing cached/local news. The app continues working when disconnected.")
        else:
