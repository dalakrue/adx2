...
def _render_connector() -> None:
    ...
    st.caption("The one canonical masked Finnhub connector is kept inside the compact sidebar to avoid duplicate controls.")
    try:
    ...
    if st.button("Open Compact Sidebar for Finnhub", key="open_sidebar_for_finnhub_20260617", use_container_width=True):
        try:
            from ui.sidebar_hard_lock import enable_native_sidebar_backup
            enable_native_sidebar_backup()
        except Exception:
