...
def _current_attempt_warning() -> None:
    ...
    if isinstance(attempt, dict) and attempt.get("calculation_status") == "FAILED":
        ...
        st.caption(
            f"Run ID {result.get('run_id','-')} • {result.get('symbol','-')} {result.get('timeframe','-')} "
            f"• Latest completed candle {(result.get('market') or {}).get('latest_completed_candle_time','-')}"
        )
...
def render_lunch_canonical_panel() -> None:
    result = _result()
    if not result:
        st.info("Run Calculation in Settings to create the synchronized validation, ledger and decision result.")
        return
...
def render_powerbi_canonical_details() -> None:
    result = _result()
    if not result:
        st.info("Run Calculation in Settings to create adaptive multi-horizon bounds.")
        return
    forecasts = dict(result.get("forecasts") or {})
    regime = dict(result.get("regime") or {})
...
def render_regime_lifecycle_panel() -> None:
    result = _result()
    if not result:
        st.info("Run Calculation in Settings to create regime lifecycle and transition evidence.")
        return
