...
def _render_full_history(ns: Dict[str, Any], result: Dict[str, Any]) -> None:
    with st.expander("Open / Close — Full Metric History — Current H1 First", expanded=False):
        ...
        if isinstance(regime_history, pd.DataFrame) and not regime_history.empty:
            ...
            try:
                st.dataframe(newest_first(regime_history, 600), use_container_width=True, hide_index=True, height=380)
            except Exception as exc:
                st.warning(f"Major regime history skipped safely: {exc}")
        ...
        if valid_factors:
            ...
            tabs = st.tabs([f"{i + 1}. {name}" for i, (name, _) in enumerate(valid_factors)])
            for tab, (name, frame) in zip(tabs, valid_factors):
                with tab:
                    st.caption(f"{name} — current/latest completed H1 first")
                    try:
                        st.dataframe(newest_first(frame, 600), use_container_width=True, hide_index=True, height=400)
                    except Exception as exc:
                        st.warning(f"{name} history skipped safely: {exc}")
