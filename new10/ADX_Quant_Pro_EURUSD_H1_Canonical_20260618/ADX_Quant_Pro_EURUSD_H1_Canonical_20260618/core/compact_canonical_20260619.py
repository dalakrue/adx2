...
def calculation_id(canonical: Mapping[str, Any]) -> str:
    existing = _first(
        canonical.get("canonical_calculation_id"),
    ...
    if existing:
        candle_hash = hashlib.sha256(str(candle).encode("utf-8", errors="ignore")).hexdigest()[:8]
        return f"{existing}:{generation or 0}:{candle_hash}"
    raw = "|".join(map(str, (signature, candle, generation, canonical.get("symbol"), canonical.get("timeframe"))))
