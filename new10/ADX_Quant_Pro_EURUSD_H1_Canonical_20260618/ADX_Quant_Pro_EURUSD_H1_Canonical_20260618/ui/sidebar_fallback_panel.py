...
def _render_connector() -> None:
    ...
    st.caption("The one canonical Finnhub key input is in Settings. This panel displays status only outside Settings.")
    try:
        from core.finnhub_connector import connection_status
        status = connection_status()
        f1, f2 = st.columns(2)
    ...
    if st.button("Finnhub key is managed above in Settings", key="finnhub_settings_owner_20260618", use_container_width=True, disabled=True):
        pass
