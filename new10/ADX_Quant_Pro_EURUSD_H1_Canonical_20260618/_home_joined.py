...
def _clean_lunch_visual_df_uncached(df, limit=1500):
    ...
    d["rolling_mean"] = close.rolling(34, min_periods=4).mean().bfill().astype("float32")
    ...
    d["ma_trend"] = close.ewm(span=144, adjust=False, min_periods=8).mean().bfill().astype("float32")
