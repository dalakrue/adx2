...
def _render_pack(pack: Dict[str, Any], selected: str) -> None:
    ...
    if selected == "Data Analysis":
        ...
        try:
            ...
            render_related_news_priority_table(nt, query="EURUSD H1 regime entry risk protect", top_n=0, window_days=25, title="📰 NLP Related News Priority — KNN + Greedy + Impact/Protect")
        except Exception as exc:
            st.caption(f"Related news priority table skipped safely: {exc}")
        ...
        if isinstance(hist, pd.DataFrame) and not hist.empty:
            ...
            st.dataframe(hist.tail(display_rows), use_container_width=True, hide_index=True, height=320)
...
def show() -> None:
    ...
    if not bool(st.session_state.get("research_run_calculate", False)):
        with st.expander("Open / Close — Research waiting", expanded=(selected != "NLP")):
            st.write("Press **Run Research Calculation** first. Heavy data mining/news/RSS logic does not run on page open.")
            st.dataframe(_lib_status(), use_container_width=True, hide_index=True, height=260)
        if selected == "Data Analysis":
            st.markdown("### 📊 Data Analysis Inner Tab — Lightweight Diagnostic Preview")
            try:
                from core.advanced_analytics_20260615 import render_diagnostic_analysis_table, render_sampling_estimation_hypothesis_panel, build_data_mining_extension_tables
                render_diagnostic_analysis_table(key="research_no_run_diag", expanded=True)
                render_sampling_estimation_hypothesis_panel(key="research_no_run_stats", expanded=False)
                with st.expander("Open / Close — Data Analysis 3D Data Cube", expanded=False):
                    cube = build_data_mining_extension_tables().get("cube")
                    st.dataframe(cube, use_container_width=True, hide_index=True, height=300)
            except Exception as exc:
                st.caption(f"Diagnostic preview skipped safely: {exc}")
        if selected == "Data Mining":
            st.markdown("### ⛏️ Data Mining Inner Tab — Lightweight Advanced Preview")
            try:
                from core.advanced_analytics_20260615 import render_data_mining_advanced_panel
                render_data_mining_advanced_panel(key="research_no_run_mining", expanded=True)
            except Exception as exc:
                st.caption(f"Data-mining preview skipped safely: {exc}")
        if selected == "NLP":
            st.markdown("### 🧠 NLP Inner Tab — Lightweight Local Pipeline")
            st.caption("Available before heavy Research run. This covers tokenization, stemming, lemmatization, stopword removal, text normalization, POS, dependency parsing, constituency parsing, NER/entity extraction, WSD, coreference, mention extraction, relation extraction, topic detection, summarization, and text generation.")
            try:
                from core.nlp_related_priority_20260615 import render_related_news_priority_table
                render_related_news_priority_table(query="EURUSD H1 regime entry risk protect", top_n=0, window_days=25, title="📰 Cached Related News Priority — KNN + Greedy + Impact/Protect")
            except Exception as exc:
                st.caption(f"Cached related-news priority skipped safely: {exc}")
            try:
                from core.nlp_lightweight_20260615 import render_nlp_pipeline_panel
                render_nlp_pipeline_panel(
                    "EURUSD H1 entry now? Compare BUY/SELL pressure, regime, TP, exit risk, alpha and delta point.",
                    key=f"research_nlp_no_run_{UNIQUE}_20260615",
                    title="Research NLP Inner Tab Pipeline",
                )
            except Exception as exc:
                st.caption(f"Local NLP pipeline skipped safely: {exc}")
        try:
            from ui.nlp_research_panel import render_nlp_research_workspace
            render_nlp_research_workspace(selected)
        except Exception as exc:
            st.caption(f"Shared NLP/data-mining workspace skipped safely: {exc}")
        return
    if st.button("🔄 Build / Refresh Research Pack", use_container_width=True, key=f"build_{UNIQUE}"):
        with st.spinner("Building Data Analysis + Data Mining + NLP research pack…"):
            _run_research(20, selected=selected)
    if "research_pack_20260612" not in st.session_state:
        with st.spinner("First research run…"):
            _run_research(20, selected=selected)
    _render_pack(st.session_state.get("research_pack_20260612", {}), selected)
    try:
        from ui.nlp_research_panel import render_nlp_research_workspace
        render_nlp_research_workspace(selected)
    except Exception as exc:
