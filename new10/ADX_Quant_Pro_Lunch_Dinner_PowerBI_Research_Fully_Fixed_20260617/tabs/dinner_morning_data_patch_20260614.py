    ...
    with st.expander("🔴 Open / Close — Lunch Red Prediction Line: smoother + error-adjusted + reliability warning", expanded=True):
        ...
        try:
            ...
            calibration = calibrate_projection_bundle(
                ...
                regime_reliability=_regime_meta_for_red_chart_v20260615().get("reliability"),
            )
        except Exception as exc:
        ...
        try:
            ...
            if isinstance(calibrated_main, pd.DataFrame) and not calibrated_main.empty:
                fig.add_trace(go.Scatter(x=calibrated_main["time"], y=calibrated_main["upper_band"], mode="lines", name="Calibrated upper band", line={"color": "rgba(124,58,237,.45)", "dash": "dash"}))
                fig.add_trace(go.Scatter(x=calibrated_main["time"], y=calibrated_main["lower_band"], mode="lines", name="Calibrated lower band", line={"color": "rgba(124,58,237,.45)", "dash": "dash"}, fill="tonexty", fillcolor="rgba(124,58,237,.09)"))
                fig.add_trace(go.Scatter(x=calibrated_main["time"], y=calibrated_main["main_path"], mode="lines+markers", name="MAIN reliability-weighted path", line={"color": "#111827", "width": 5, "shape": "spline", "smoothing": 1.0}, marker={"size": 7}))
            ...
            if not red_display.empty:
                fig.add_trace(go.Scatter(x=red_display["time"], y=red_display["path"], mode="lines+markers", name="RED calibrated model path", line={"color": "#DC2626", "width": 3}, marker={"size": 5}))
            if isinstance(smooth_red, pd.DataFrame) and not smooth_red.empty:
                fig.add_trace(go.Scatter(x=smooth_red["time"], y=smooth_red["red_path"], mode="lines", name="RED smooth calibrated path", line={"color": "#EF4444", "width": 4, "shape": "spline", "smoothing": 1.05}))
...
def _render_original_data_last(ns: dict, prev_data) -> None:
    with st.expander("📦 Open / Close — Original Data / Advanced Details at Last Place", expanded=False):
        ...
        if ready and callable(prev_data):
            ...
            st.info("Run Calculation in Settings once. This deepest advanced section will then load and stay synchronized automatically.")
        else:
...
def _render_dinner(ns: dict, prev_data) -> None:
    ...
    options = ["Unified Regime + Logic", "AI Assistant"]
    active = st.session_state.get("dinner_active_inner_20260615", "Unified Regime + Logic")
    if active not in options:
        active = "Unified Regime + Logic"
    try:
