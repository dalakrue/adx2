...
def _ai_20260615_commit(prompt: str, ctx: Dict[str, Any], *, category: str = "Free Chat", answer_mode: str = "Auto", advanced: bool = False) -> None:
...
def _ai_20260615_render_chat(ctx: Dict[str, Any]) -> None:
    ...
    if selected_label and selected_label != labels[0] and selected_label != st.session_state.get("ai_lite_last_choice_answered_20260615"):
        ...
        _ai_20260615_commit(prompt, ctx, category=category, answer_mode="Auto")
...
def _ai_20260615_render_data_mining(ctx: Dict[str, Any]) -> None:
    ...
    try:
        ...
        if isinstance(merged, pd.DataFrame) and not merged.empty:
            phone_rows = 168 if bool(st.session_state.get("phone_mode", False)) else 360
            st.dataframe(merged.tail(phone_rows), use_container_width=True, hide_index=True, height=420)
    except Exception as exc:
...
try:
    ...
    def build_ai_context_from_existing_data() -> Dict[str, Any]:  # type: ignore[no-redef]
        ctx = _ai_lite_original_build_context_20260615()
        try:
            shared = _adx_ensure_shared_20260615()
            if isinstance(shared, dict):
                current = ctx.setdefault("current", {})
                scurrent = shared.get("current", {}) if isinstance(shared.get("current"), dict) else {}
                # Fill missing local AI context from the shared calculation only.
                for key in [
                    "symbol", "timeframe", "decision", "regime", "regime_direction", "prediction_direction",
                    "last_close", "forecast_close", "entry_score", "hold_score", "tp_score", "exit_risk",
                    "forecast_confidence", "market_quality",
                ]:
                    if current.get(key) in (None, "", "-", "data not available yet") and scurrent.get(key) not in (None, "", "-"):
                        current[key] = scurrent.get(key)
                ...
                ad = shared.get("regime_alpha_delta", {}) if isinstance(shared.get("regime_alpha_delta"), dict) else {}
                q = shared.get("data_quality", {}) if isinstance(shared.get("data_quality"), dict) else {}
                g = shared.get("ai_grounding", {}) if isinstance(shared.get("ai_grounding"), dict) else {}
                # Canonical grounding is authoritative; the AI renderer cannot
                # infer or retain a conflicting BUY/SELL/WAIT result.
                if g:
                    current["decision"] = g.get("decision", current.get("decision", "DATA NOT READY"))
                    current["less_risky_decision"] = g.get("less_risky_decision", current.get("less_risky_decision", "WAIT"))
                    current["prediction_direction"] = g.get("prediction_direction", current.get("prediction_direction", "WAIT"))
                    current["regime"] = g.get("regime", current.get("regime", "UNKNOWN"))
                    current["selected_horizon"] = g.get("selected_horizon", current.get("selected_horizon", 3))
                    current["run_id"] = g.get("run_id")
                    current["calculation_generation"] = g.get("calculation_generation")
                current.setdefault("prediction_reliability", rel.get("score"))
                current.setdefault("prediction_vs_actual_error", fb.get("avg_abs_close_error_pct"))
                current.setdefault("direction_accuracy", fb.get("direction_accuracy_pct"))
                # Reuse the same canonical display snapshot as Lunch/PowerBI.
                # This replaces stale zero-error placeholders and stale regime labels.
                try:
                    from core.regime_sync_20260617 import canonical_regime_snapshot
                    canonical = canonical_regime_snapshot(days=25)
                    current["regime"] = canonical.get("regime", current.get("regime"))
                    current["regime_direction"] = canonical.get("regime_direction", current.get("regime_direction"))
                    current["decision"] = canonical.get("decision", current.get("decision"))
                    current_error = _num(current.get("prediction_vs_actual_error"), None)
                    if current_error is None or current_error <= 1e-9:
                        current["prediction_vs_actual_error"] = canonical.get("avg_error_pct")
                    current["regime_start"] = canonical.get("regime_start")
                    current["regime_end"] = canonical.get("regime_end_display")
                    current["regime_validation"] = canonical.get("regime_validation")
                except Exception:
                    pass
                ctx["shared_calc_result"] = shared
                ctx["ai_grounding"] = g
                ctx["reliability_calibration"] = rel
                ctx["prediction_feedback"] = fb
                ctx["regime_alpha_delta"] = ad
                ctx["data_quality"] = q
                ctx["data_available"] = bool(ctx.get("data_available") or q.get("rows", 0) > 0)
                missing = set(ctx.get("missing_fields", []))
                for filled in ["last_close", "forecast_confidence", "entry_score", "tp_score", "exit_risk"]:
                    if current.get(filled) not in (None, "", "-") and filled in missing:
                        missing.remove(filled)
                ctx["missing_fields"] = list(missing)
        except Exception as exc:
            ctx.setdefault("shared_sync_error", str(exc))
        return ctx
