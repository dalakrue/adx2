...
DINNER_CHILDREN = ["Unified Regime + Logic", "AI Assistant"]
RESEARCH_CHILDREN = ["AI Assistant", "KNN / Greedy", "Quant Structure"]
SUBPAGE_PARENT: Dict[str, str] = {
    **{x: "Lunch" for x in LUNCH_CHILDREN},
    "Unified Regime + Logic": "Dinner", "Regime Summary": "Dinner", "Combine Logic": "Dinner",
    "KNN / Greedy": "Research", "Quant Structure": "Research",
}
...
def safe_antd_navigation(location_key: str = "antd_main_navigation") -> Tuple[str, str]:
    ...
    if selected == "AI Assistant":
        parent = _normalize_page(st.session_state.get("active_page"))
        parent = parent if parent in {"Dinner", "Research"} else "Research"
        return _sync_legacy_state(parent, selected)
