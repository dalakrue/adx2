...
def fetch_market_news(category: str = "forex", *, force: bool = False, ttl_seconds: int = 900) -> List[Dict[str, Any]]:
    """Fetch Finnhub news with session-only TTL caching and safe fallback."""
    initialize_finnhub_state()
    key = get_session_key()
    ...
    if not force and isinstance(cached, list) and cached and age < max(60, int(ttl_seconds)):
        return cached

    requested = str(category or "forex").strip().lower() or "forex"
    preferred = str(st.session_state.get(NEWS_MODE_STATE, "forex") or "forex")
    categories = []
    for item in (requested if preferred == "forex" else preferred, "general"):
    ...
    for current_category in categories:
        try:
            ...
            safe_rows = _safe_news_rows(payload, category=current_category)
            st.session_state[NEWS_STATE] = safe_rows
            st.session_state[NEWS_TIME_STATE] = time.time()
            st.session_state[NEWS_MODE_STATE] = current_category
            st.session_state[ERROR_STATE] = ""
            ...
            if exc.rate_limited:
                ...
                break
            # Continue from forex to general on entitlement/category failure.
            continue
        except Exception as exc:
            last_error = exc
            continue

    st.session_state[ERROR_STATE] = _redact(last_error or "Finnhub news request failed", key)
