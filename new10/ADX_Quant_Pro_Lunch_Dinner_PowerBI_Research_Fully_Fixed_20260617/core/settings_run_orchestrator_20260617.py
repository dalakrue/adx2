...
def _source_identity() -> tuple[str, str, str]:
    symbol = str(st.session_state.get("symbol") or st.session_state.get("selected_symbol") or "EURUSD")
    timeframe = str(st.session_state.get("timeframe") or st.session_state.get("selected_timeframe") or "H1")
...
def run_settings_calculation(ns: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        try:
            ...
            canonical = serialize_result(canonical_obj)
