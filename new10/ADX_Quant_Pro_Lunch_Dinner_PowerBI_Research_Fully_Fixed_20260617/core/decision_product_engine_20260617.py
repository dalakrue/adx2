...
def validate_data_quality(
    ...
    future_rows = int((frame["time"] > now_ts + pd.Timedelta(minutes=5)).sum()) if not frame.empty else 0
    if future_rows: blocking.append(f"Future timestamps detected: {future_rows}")
    ...
    severe = any(x for x in blocking if any(k in x.lower() for k in ("missing required", "no completed", "symbol identity", "future timestamps")))
    if severe:
        status = "FAIL_ALL"
    elif blocking:
        status = "FAIL_MODEL"
...
def _actionability(history: pd.DataFrame, *, probability: Optional[float], interval_ratio: Optional[float], priority: float, expected_value: Optional[float], transition: float, drift: DriftResult, quality: DataQualityResult, nlp: NLPResult) -> Tuple[float,str,str,List[str],str]:
    ...
    if len(history) >= 60 and "direction_correct" in history:
        try:
            ...
            else: raise ValueError("one class")
        except Exception:
            prob = 0.42*p + 0.25*(priority/100) + 0.18*(1-transition) + 0.15*quality.score/100; source="DETERMINISTIC_FALLBACK"
    else:
        prob = 0.42*p + 0.25*(priority/100) + 0.18*(1-transition) + 0.15*quality.score/100; source="DETERMINISTIC_FALLBACK"
    if interval_ratio is not None: prob -= min(0.20, interval_ratio*20)
    if expected_value is not None and expected_value <= 0: prob -= 0.25; blockers.append("expected value is not positive")
    if drift.status == "DEGRADED": prob -= 0.10
    if drift.status == "CRITICAL": prob -= 0.25; blockers.append("critical drift")
    if quality.status.startswith("FAIL"): prob = 0; blockers.append("data quality failed")
    if nlp.conflict_level in {"HIGH","CRITICAL"} and nlp.importance >= 0.65: prob -= 0.12; blockers.append("important NLP conflict")
    prob = _clip(prob)
    label = "YES" if prob >= 0.58 and not blockers else "NO"
    reason = "Signal passes actionability evidence" if label=="YES" else (blockers[0] if blockers else "Evidence is below actionability gate")
...
def build_decision_result(
    *, legacy_shared: Dict[str,Any], ohlc: pd.DataFrame, symbol: str="EURUSD", timeframe: str="H1",
    ...
    for h,direction in zip(HORIZONS,preliminary_dirs):
        ...
        blockers=list(action_blockers)
        if direction_p is None: blockers.append("directional probability unavailable")
        elif direction_p<threshold: blockers.append(f"probability {direction_p:.3f} below threshold {threshold:.3f}")
        if ev["expected_value"] is None or ev["expected_value"]<=0: blockers.append("expected value not positive")
        if interval_ratio is None or interval_ratio>0.008: blockers.append("prediction interval unacceptable")
        if quality.status.startswith("FAIL"): blockers.append("data quality did not pass")
        if drift.status=="CRITICAL": blockers.append("critical drift")
        if nlp.conflict_level in {"CRITICAL"} and nlp.importance>=0.65: blockers.append("critical NLP conflict")
        decision=direction if direction in {"BUY","SELL"} and not blockers and action_label=="YES" else "WAIT"
    ...
    if primary.decision=="WAIT" and horizons["2h"].decision in {"BUY","SELL"}: selected=2; primary=horizons["2h"]
    final_blockers=list(primary.blocking_reasons)
    if label=="LARGE DISAGREEMENT": final_blockers.append("large multi-horizon disagreement")
    final_decision=primary.decision if primary.decision in {"BUY","SELL"} and label!="LARGE DISAGREEMENT" else ("DATA NOT READY" if quality.status=="FAIL_ALL" else "WAIT")
    market_view=primary.direction
    confidence={"BUY":primary.buy_probability_calibrated,"SELL":primary.sell_probability_calibrated,"WAIT":primary.wait_probability_calibrated}.get(market_view)
    ...
    supporting=[f"{selected}h market view {market_view}",f"horizon agreement {agreement_score:.0%}",f"priority {priority.label} {priority.score:.1f}"]
    main_reason=(f"{selected}h signal passed calibrated probability, cost, interval and actionability gates" if final_decision in {"BUY","SELL"} else (final_blockers[0] if final_blockers else reason))
    final=FinalDecision(
        final_decision=final_decision,directional_market_view=market_view,tradeability_decision=final_decision if final_decision in {"BUY","SELL"} else "WAIT",less_risky_decision=final_decision if final_decision in {"BUY","SELL"} else "WAIT",main_reason=main_reason,supporting_reasons=supporting,blocking_reasons=sorted(set(final_blockers)),calibrated_confidence=confidence,actionability_probability=primary.actionability_probability,expected_value=primary.expected_value,risk_level=risk_level,uncertainty_pct=uncertainty,error_estimate_pct=error_pct,selected_horizon=selected,decision_expiry_time=primary.due_time,regime_transition_warning=regime.transition_warning,drift_warning=drift.status,data_quality_warning=quality.status,
    )
