...
def _short_lines(canonical: Mapping[str, Any], summary: Mapping[str, Any], plan: Mapping[str, Any], state: Mapping[str, Any] | None = None) -> list[tuple[str, bool]]:
    ...
    generation = canonical.get("run_id") or canonical.get("canonical_calculation_id") or summary.get("calculation_id")
    ...
    return [
        (f"ADX Quant Pro — Copy Short | Symbol/timeframe: {_text(symbol)} {_text(timeframe)} | Current price: {_text(price, 'Not published')} | Generation ID: {_text(generation, 'Not published')}", True),
