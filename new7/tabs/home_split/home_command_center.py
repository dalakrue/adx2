...
def render_home_command_center() -> None:
    ...
    with action_cols[1]:
        if st.button("🏦 Read account now", use_container_width=True, key="home_cmd_read_account"):
            ...
            _safe_rerun()
    with action_cols[2]:
        if st.button("💾 Save Home snapshot", use_container_width=True, key="home_cmd_save"):
            _save_home_snapshot(q, market, quality, action)
    with action_cols[3]:
    ...
    with st.expander("📋 Copy compact Home Command Center report", expanded=False):
        ...
        st.download_button("Download command report", text, file_name="home_command_report.txt", mime="text/plain", use_container_width=True, key="home_cmd_download")
