...
def add_snapshot_button(location: str = "global") -> None:
    if st.button("📌 Save screen snapshot", use_container_width=True, key=f"{location}_save_snapshot"):
        row: Dict[str, Any] = {
            ...
            "source": st.session_state.get("source", "DISCONNECTED"),
            "rows": _safe_len(get_shared_market_df()),
        }
        try:
            from core.database import append_csv
            append_csv("system_snapshots", row)
            st.success("Snapshot saved to data/system_snapshots.csv")
        except Exception as exc:
            st.warning(f"Snapshot could not be saved: {exc}")
