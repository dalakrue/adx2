...
def run_guarded_startup(state: MutableMapping[str, Any], home_namespace: Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    result["newer_h1_available"] = bool(newer)
    result["auto_calculated"] = False
    result["status"] = "MANUAL_CALCULATION_REQUIRED" if newer else ("CURRENT" if latest is not None else "DATA_NOT_READY")
    result["message"] = (
        "A newer H1 is connected. Use Settings → Run Calculation + Open Lunch once."
        if newer else "Startup connection completed; no automatic calculation was run."
    )
    state["secure_startup_status_20260619"] = result
    return result
