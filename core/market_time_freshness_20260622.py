...
def market_time_snapshot(
    ...
    *,
    ...
    broker_offset = broker_offset_hours(state)
    broker_clock = to_display_clock(tick or now, offset_hours=broker_offset)
    myanmar_clock = to_display_clock(now, offset_hours=MYANMAR_UTC_OFFSET_HOURS)
    latest_broker = to_display_clock(latest, offset_hours=broker_offset)
    ...
    return {
        ...
        "latest_loaded_broker_display": _clock_text(latest_broker, broker_offset, "Broker"),
        ...
        "broker_clock_display": _clock_text(broker_clock, broker_offset, "Broker"),
