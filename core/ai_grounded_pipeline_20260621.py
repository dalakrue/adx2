...
def answer_question(question: str, *, canonical: Mapping[str, Any], summary: Mapping[str, Any], plan: Mapping[str, Any] | None, state: MutableMapping[str, Any]) -> dict[str, Any]:
    ...
    registry = build_source_registry(canonical, summary, plan, settled, max_records=budget.max_registry_records, state=state)
    ...
    regime = _m(summary.get("regime"))
    priority = _m(summary.get("priority"))
    final_decision = _m(canonical.get("final_decision"))
    final += "\n" + "\n".join([
        f"**Current regime:** {regime.get('directional_regime') or _m(canonical.get('regime')).get('major_regime') or 'UNKNOWN'}",
        f"**Current reliability:** {regime.get('regime_reliability') or _m(canonical.get('reliability')).get('score') or 'Not published'}",
        f"**Current priority:** {priority.get('opportunity_quality') or priority.get('knn_priority') or priority.get('current_rank') or 'Not published'}",
        f"**Reasoning summary:** Answer classified as {intent.replace('_', ' ')} and restricted to {', '.join(required)} evidence.",
    ])
    ...
    duplicate_guard = prevent_duplicate_answer(state, question=question, answer=final, intent=intent, evidence=evidence)
    final = str(duplicate_guard.get("answer") or final)
    remember(state, question=question, intent=intent, generation_id=generation_id, evidence=evidence, status=str(critic.get("status")), answer=final)
    return {
        "answer": final,
